...
def _getdirections(bot, event, text, type):
    ...
    regionbias = bot.get_config_option("direction_geobias")
    if not bot.get_config_option("direction_geobias"):
        logger.warning("""No geobias loaded... add IANA region code to config file " direction_geobias" to remove this warning """)
        logger.info("setting geobias to null")
        regionbias = " "
    ...
    for s in text.sentences:
        ...
        if all(x in s.lower() for x in type):
            ...
            try:
                ...
                mapsUrl = "https://www.google.com/maps/dir/" + quote(startAddr) + "/" + quote(endAddr) + "/data=!4m2!4m1!3e"
                routeUrlParams = {"walking":"w","transit":"r","bicycling":"b","driving":" "}
                logger.info(dirs1)
                logger.info(dirleg)
                logger.info(dirs)
                if routeMode:
                  mapsUrl += "&dirflg=" + routeUrlParams[routeMode]
                else:
                  mapsUrl += '0'
