...
def _initialise(bot):
    plugins.register_user_command(["gtldr"])


def gtldr(bot, event, *args):
    """Adds a short message to a list saved globally using:
    /bot gtldr <message>
    All GTLDRs can be retrieved by /bot gtldr, single tldr with /bot gtldr <number>
    All GTLDRs can be deleted using /bot gtldr clear, single tldr with /bot gtldr clear <number>"""
    parameters = list(args)

    if not bot.memory.exists(['tldr']):
        bot.memory.set_by_path(['tldr'], {})

    if not bot.memory.exists(['tldr', 'GLOBAL']):
        bot.memory.set_by_path(['tldr', 'GLOBAL'], {})

    conv_tldr = bot.memory.get_by_path(['tldr', 'GLOBAL'])
    ...
    if display is not False:
        ...
        if len(html) == 0:
            html.append(_("GTL;DR not found"))
        else:
            html.insert(0, _("<b>GTL;DR ({} stored):</b>").format(len(conv_tldr)))
    ...
    if parameters[0] == "clear":
        if len(parameters) == 2 and parameters[1].isdigit():
            ...
            if key_index < 0 or key_index >= len(sorted_keys):
                message = _("GTL;DR #{} not found").format(parameters[1])
            else:
                popped_tldr = conv_tldr.pop(sorted_keys[key_index])
                bot.memory.set_by_path(['tldr', 'GLOBAL'], conv_tldr)
                message = _('GTL;DR #{} removed - "{}"').format(parameters[1], popped_tldr)
        else:
            bot.memory.set_by_path(['tldr', 'GLOBAL'], {})
            message = _("All GTL;DRs cleared")
        ...
        if tldr:
            ...
            yield from bot.coro_send_message( event.conv_id,
                                              _('Added "{}" to GTL;DR. Count: {}').format( tldr,
                                                                                          len(conv_tldr) ))
